...
describe("RestrictBySignatureV3 Zone tests", function () {
  ...
  describe("erc20<->erc20", function () {
    /*
        ...
        - tests new V3 feature - erc1271 signer (contract signer)
        - using safe as example erc1271 signer
    */
    it("erc1271 signer (safe)", async function () {
      const {
        alice,
        bob,
        charlie,
      ...
      // aliceSafe will designate that only bob and charlie can fill the trade
      ...
      // bob and charlie approve seaport contract
      await (
        await getWeth(bob)
      ...
      // aliceSafe signs the merkle root and order hash
      const sigDomainData = {
      ...
      const rootDataHash = hashTypedData({
      ...
      const rootSignature = await signMessageAndValidate(
        [alice],
        rootDataHash,
        aliceSafe.address
      );
      ...
      ).to.be.rejectedWith("reverted with reason string 'GS026'");
